...
export default function LeaveRequestManagement({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <TableRow key={r.id} className="dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end items-center gap-2">
                    ...
                    {onDeleteRequest && (isAdmin || r.staffId === currentUser?.id) && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 text-slate-500 hover:text-red-600 hover:bg-red-50 dark:text-slate-400 dark:hover:text-red-400 dark:hover:bg-red-900/20"
                        title="Xóa yêu cầu"
      ...
      <Dialog open={requestToDelete !== null} onOpenChange={(open) => { if (!open) setRequestToDelete(null); }}>
        <DialogContent className="max-w-[400px]">
          ...
          <div className="py-2 text-sm text-slate-600 dark:text-slate-400">
            Bạn có chắc chắn muốn xóa yêu cầu nghỉ phép này? Hành động này không thể hoàn tác.
          </div>
          <DialogFooter className="flex gap-2 justify-end mt-4">
            <Button variant="outline" size="sm" onClick={() => setRequestToDelete(null)}>
              Hủy
            </Button>
            <Button
              variant="destructive"
